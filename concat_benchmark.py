...
def print_3visaconcat_benchmark():
    ...
    print(f'TOTAL TIME: {(time()-t0)*100/60} min')
    print('################## END {} #########################'.format(benchmark_name))

def print_3visaconcat_benchmark_multicore():
    benchmark_name = 'ALL CONCAT MULTI CORE (in minutes)'
    print('\n################## START {} #########################'.format(benchmark_name))
    bboxes = np.array([
        [ 945,   70,  400,   75],
        [ 250, 1110,  350,  100],
    ...
    def predict_one_image():
        with PyTessBaseAPI(psm=PSM.SINGLE_BLOCK, oem=OEM.LSTM_ONLY, lang='eng') as api:
            ...
            _ = api.GetUTF8Text()
    t0 = time()
    ray.get([predict_one_image.remote() for _ in range(100)])
    print(f'TOTAL TIME: {(time()-t0)*100/60} min')
    print('################## END {} #########################'.format(benchmark_name))


def print_3visaconcat_benchmark_multicore_with_10_tesseracts():
    benchmark_name = 'ALL CONCAT MULTI CORE W/ 10 TESSERACTS (in minutes)'
    print('\n################## START {} #########################'.format(benchmark_name))
    bboxes = np.array([
        [ 945,   70,  400,   75],
        [ 250, 1110,  350,  100],
    ...
    def predict_one_image():
        with PyTessBaseAPI(psm=PSM.SINGLE_BLOCK, oem=OEM.LSTM_ONLY, lang='eng') as api:
            ...
            for _ in range(10):
                ...
                _ = api.GetUTF8Text()
    t0 = time()
    ray.get([predict_one_image.remote() for _ in range(10)])
    print(f'TOTAL TIME: {(time()-t0)*100/60} min')
    print('################## END {} #########################'.format(benchmark_name))